#ifndef TESTBINARLIB_BENCHMARK_HPP
#define TESTBINARLIB_BENCHMARK_HPP
...
namespace benchmark {
    ...
    public:
        ...
        ~iteration_controller() {
            ...
            m_lastResult =
                    (std::chrono::duration_cast<time_t>(end - m_startTime) / m_iterations) - m_emptyRuns[m_iterations];
        }
    ...
    class metric_collector {
        ...
        static std::string dashIfEmpty(const std::string &string) {
            return (string.empty() || string.empty() || string == " ") ? "-" : string;
        }

    public:
        explicit metric_collector(std::string format, const std::string &implementation, std::string id)
                : m_format(std::move(format)), m_implementation(dashIfEmpty(implementation)), m_id(std::move(id)) {}

        ~metric_collector() {
            const double seconds = std::chrono::duration_cast<std::chrono::duration<double>>(lastResult()).count();
            std::cout << "value: " << m_format << ' ' << m_implementation << ' ' << m_id << ' ' << seconds << std::endl;
        }
...
#endif //TESTBINARLIB_BENCHMARK_HPP
